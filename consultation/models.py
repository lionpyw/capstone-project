...
class Client(models.Model):
    GENDER = [("M", "Masculine"), ("F", "Feminine"), ("O", "Other")]
...
class Service(models.Model):
    ...
    category = models.CharField(
        max_length=2, choices=SERVICE_CATEGORY, default="OR"
    )
...
class Consultant(models.Model):
    ...
    profile_img = models.ImageField(
        upload_to="consult/images", default="consult/default.jpg"
    )
    session_charge = models.DecimalField(
...
class AppointmentRequest(models.Model):
    client = models.ForeignKey(
        Client, on_delete=models.CASCADE, related_name="appointment_req"
    )
    consultant = models.ForeignKey(
        Consultant, on_delete=models.CASCADE, related_name="termin"
    )
...
class Appointment(models.Model):
    appointment = models.OneToOneField(
        AppointmentRequest, on_delete=models.CASCADE, related_name="schedule"
    )
...
class ClientDocument(models.Model):
    appointment = models.OneToOneField(
        Appointment, on_delete=models.PROTECT, related_name="docs"
    )
    documents = models.FileField(
        upload_to="consult/docs", null=True, blank=True
    )
...
class LiveVideoRequest(models.Model):
    ...
    consultant = models.ForeignKey(
        Consultant, on_delete=models.DO_NOTHING, related_name="live"
    )
