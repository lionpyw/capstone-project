...
class ClientsViewSet(viewsets.ModelViewSet):
    ...
    def me(self, request):
        client = Client.objects.get(client_id=request.user.id)
...
class ConsultantsViewSet(viewsets.ModelViewSet):
    queryset = (
        Consultant.objects.select_related("consultant")
    ...
    @action(
        detail=False, methods=["GET", "PUT"], permission_classes=[IsConsultant]
    )
    def me(self, request):
        consultant = Consultant.objects.get(consultant_id=request.user.id)
        if request.method == "GET":
...
class AppointmentRequestListView(
    ...
    def get_queryset(self):
        ...
        return (
            AppointmentRequest.objects.select_related("consultant__consultant")
            .select_related("client__client")
            .select_related("consultant__service")
            .filter(consultant__consultant_id=user.id)
            .all()
            .order_by("-proposed_time")
        )
...
class AppointmentRequestViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            ...
            .select_related("consultant__service")
            .filter(
                Q(consultant__consultant_id=user.id)
                | Q(client__client_id=user.id)
            )
            .all()
        )
    ...
    def get_queryset(self):
        ...
        return (
            Appointment.objects.select_related(
                "appointment__consultant__service"
            )
            .select_related("appointment__consultant__consultant")
            .select_related("appointment__client__client")
            .filter(
                Q(appointment__consultant__consultant_id=user.id)
                | Q(appointment__client__client_id=user.id)
            )
            .all()
        )
...
class ClientDocumentsViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        apps = Appointment.objects.filter(
            appointment__client__client_id=user.id
        ).all()
        if user.is_staff:
            return ClientDocument.objects.all()
        client_doks = [
            ClientDocument.objects.filter(appointment=app).all()
            for app in apps
        ]
        return client_doks
...
class LiveVideoRequestViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if serializer.is_valid():
            consultant: Consultant = serializer.validated_data.get(
                "consultant"
            )
    ...
    def me(self, request, pk=None, url_path="live-requests/(?P<pk>[^/.]+)"):
        live_req = LiveVideoRequest.objects.get(consultant__id=pk)
        if request.method == "GET":
    ...
    def start(self, request, pk=None):
        ...
        live_request.save()
        return Response(
            {"dyte_auth_token": participant["token"]},
